...
def load_from_file():
    """Loads words from a user-specified .txt or .csv file, checking the current and script directory."""
    clear_screen()
    filename_input = input("Enter filename (e.g., words.txt) or path to your file: ").strip()

    filepath = None
    # Check 1: Does the path exist as given (absolute or relative to current dir)
    if os.path.exists(filename_input):
        filepath = filename_input
    else:
        # Check 2: If not, does it exist in the same directory as the script?
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            path_in_script_dir = os.path.join(script_dir, filename_input)
            if os.path.exists(path_in_script_dir):
                filepath = path_in_script_dir
        except NameError:
            # __file__ is not defined in some environments, so we can't check the script dir.
            pass
...
def start_spelling_test(words):
    ...
    words_to_test = list(words) # Make a copy to not modify the original list

    while True: # This loop allows for re-testing misspelled words
        test_results = []
        for i, word in enumerate(words_to_test):
            ...
            speak(word, rate=130) # Repeat the word for clarity
        ...
        if not misspelled_words:
            ...
            if practice_again == 'y':
                # Set up the next round with only the misspelled words
